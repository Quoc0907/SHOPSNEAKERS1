        ...
        <form action="{{ route('cart.update') }}" method="POST">
            ...
            <table style="width:100%; border-collapse:collapse; margin-bottom:30px;">
                ...
                @foreach($cart as $id => $item)
                    @php $subtotal = $item['QTY'] * $item['GIA']; $total += $subtotal; @endphp
                    <tr>
                        ...
                        <td style="padding:10px; border:1px solid #ddd;">
                            <input type="number" name="cart[{{ $id }}][qty]" value="{{ $item['QTY'] }}" min="1" style="width:60px; padding:5px;">
                        </td>
                        <td style="padding:10px; border:1px solid #ddd;">{{ number_format($item['GIA']) }}₫</td>
                        <td style="padding:10px; border:1px solid #ddd;">{{ number_format($subtotal) }}₫</td>
                        <td style="padding:10px; border:1px solid #ddd;">
                            <a href="{{ route('cart.delete', $id) }}" style="color:red;">Xóa</a>
                        </td>
                    </tr>
                @endforeach
                <tr>
                    <td colspan="5" style="text-align:right; font-weight:bold; padding:10px;">Tổng cộng:</td>
                    <td style="padding:10px; font-weight:bold;">{{ number_format($total) }}₫</td>
                    <td></td>
                </tr>
            </table>
            <button type="submit" style="padding:10px 20px; background:#ffc107; border:none; border-radius:5px; cursor:pointer; margin-bottom:30px;">Cập nhật giỏ hàng</button>
        </form>

        <form action="{{ route('payment.index') }}" method="POST" style="display:flex; flex-direction:column; gap:15px;">
            @csrf
            <h3>Thông tin khách hàng</h3>
            <input type="text" name="name" placeholder="Họ tên" value="{{ $user->HOTEN ?? '' }}" required style="padding:10px; border:1px solid #ddd; border-radius:5px;">
            <input type="email" name="email" placeholder="Email" value="{{ $user->EMAIL ?? '' }}" required style="padding:10px; border:1px solid #ddd; border-radius:5px;">
            <input type="text" name="phone" placeholder="Số điện thoại" value="{{ $user->SDT ?? '' }}" required style="padding:10px; border:1px solid #ddd; border-radius:5px;">
